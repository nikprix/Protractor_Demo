var BasePage = function () {
     ...
     * @returns {promise}
     * @requires a page to include `pageLoaded` method in child class!!!
     */
    this.at = function () {
        return browser.wait(this.pageLoaded(), this.timeout.xxl, 'Page was not loaded within given time!');
    };
    ...
    this.waitforElem = function (locator) {
        browser.wait(EC.presenceOf(locator), this.timeout.s);
    };

    this.hoverOverElem = function (locator) {
        browser.actions().mouseMove(locator).perform();
    };

    this.pauseTest = function (timeout) {
    ...
    /**
     * Webdriver equivilant to hitting ESC key.
     */
    this.hitEsc = function () {
